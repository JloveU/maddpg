...
tf_major_version, tf_minor_version = int(tf.__version__.split(".")[0]), int(tf.__version__.split(".")[1])


if (tf_major_version >= 1) and (tf_minor_version >= 5):
    ...
    def sum(x, axis=None, keepdims=False):
        return tf.reduce_sum(x, dim=None if axis is None else [axis], keep_dims=keepdims)
    def mean(x, axis=None, keepdims=False):
        return tf.reduce_mean(x, dim=None if axis is None else [axis], keep_dims=keepdims)
    def var(x, axis=None, keepdims=False):
        meanx = mean(x, axis=axis, keepdims=keepdims)
        return mean(tf.square(x - meanx), axis=axis, keepdims=keepdims)
    def std(x, axis=None, keepdims=False):
        return tf.sqrt(var(x, axis=axis, keepdims=keepdims))
    def max(x, axis=None, keepdims=False):
        return tf.reduce_max(x, dim=None if axis is None else [axis], keep_dims=keepdims)
    def min(x, axis=None, keepdims=False):
        return tf.reduce_min(x, dim=None if axis is None else [axis], keep_dims=keepdims)
    def concatenate(arrs, axis=0):
        return tf.concat(dim=axis, values=arrs)
    def argmax(x, axis=None):
        return tf.argmax(x, dim=axis)
    def softmax(x, axis=None):
